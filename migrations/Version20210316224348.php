...
final class Version20210316224348 extends AbstractMigration
{
    public function up(Schema $schema): void
    {
        $this->addSql('CREATE TABLE funding (id UUID NOT NULL, user_id UUID DEFAULT NULL, gateway VARCHAR(15) NOT NULL, paypal_order_id VARCHAR(31) DEFAULT NULL, paypal_status VARCHAR(31) DEFAULT NULL, paypal_purchase JSON DEFAULT NULL, amount INT DEFAULT 0 NOT NULL, net_amount INT DEFAULT 0 NOT NULL, currency CHAR(3) NOT NULL, created_at TIMESTAMP(0) WITH TIME ZONE NOT NULL, refunded_amount INT DEFAULT 0 NOT NULL, refunded_net_amount INT DEFAULT 0 NOT NULL, refunded_at TIMESTAMP(0) WITH TIME ZONE DEFAULT NULL, PRIMARY KEY(id))');
        ...
        $this->addSql('COMMENT ON COLUMN funding.id IS \'(DC2Type:uuid)\'');
        ...
        $this->addSql('COMMENT ON COLUMN monthly_cost_coverage.id IS \'(DC2Type:uuid)\'');
        $this->addSql('COMMENT ON COLUMN monthly_cost_coverage.month IS \'(DC2Type:date_immutable)\'');
        $this->addSql('CREATE TABLE patch_note (id UUID NOT NULL, title VARCHAR(255) NOT NULL, body TEXT NOT NULL, link VARCHAR(255) DEFAULT NULL, created_at TIMESTAMP(0) WITH TIME ZONE NOT NULL, updated_at TIMESTAMP(0) WITH TIME ZONE NOT NULL, PRIMARY KEY(id))');
        $this->addSql('CREATE INDEX created_at_idx ON patch_note (created_at)');
        $this->addSql('COMMENT ON COLUMN patch_note.id IS \'(DC2Type:uuid)\'');
        $this->addSql('COMMENT ON COLUMN patch_note.created_at IS \'(DC2Type:datetimetz_immutable)\'');
        $this->addSql('COMMENT ON COLUMN patch_note.updated_at IS \'(DC2Type:datetimetz_immutable)\'');
        $this->addSql('CREATE TABLE "user" (id UUID NOT NULL, roles JSON NOT NULL, auth0_username VARCHAR(127) NOT NULL, supporter_visible BOOLEAN DEFAULT \'true\' NOT NULL, coins INT DEFAULT 0 NOT NULL, created_at TIMESTAMP(0) WITH TIME ZONE NOT NULL, last_patch_note_read_at TIMESTAMP(0) WITH TIME ZONE DEFAULT NULL, PRIMARY KEY(id))');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649F28DC078 ON "user" (auth0_username)');
        ...
        $this->addSql('COMMENT ON COLUMN "user".last_patch_note_read_at IS \'(DC2Type:datetimetz_immutable)\'');
        $this->addSql('ALTER TABLE funding ADD CONSTRAINT FK_D30DD1D6A76ED395 FOREIGN KEY (user_id) REFERENCES "user" (id) NOT DEFERRABLE INITIALLY IMMEDIATE');
    }

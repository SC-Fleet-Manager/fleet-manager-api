...
class CitizenRepository extends ServiceEntityRepository
{
     ...
     * @return Citizen[]
     */
    public function findVisiblesSomeHandlesWithLastFleet(array $handles, string $organizationId, ?Citizen $viewerCitizen): array
    {
        ...
        $sql = <<<EOT
            SELECT c.*, c.id AS citizenId, f.*, f.id AS fleetId, s.*, s.id AS shipId
            ...
            WHERE (
                LOWER(c.actual_handle) IN (:handles) AND (

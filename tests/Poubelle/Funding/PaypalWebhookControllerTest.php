...
class PaypalWebhookControllerTest extends WebTestCase
{
    ...
    public function test_partial_refund(): void
    {
        ...
        $mock = $this->getMockBuilder(VerifyWebhookSignature::class)
            ...
            ->getMock();
        $mock->expects(static::once())->method('post')->willReturn(new VerifyWebhookSignatureResponse(['verification_status' => 'SUCCESS']));
     ...
     * @group funding
     */
    public function test_multiple_full_refund(): void
    {
        $verifyWebhookSignature = static::$container->get(MockVerifyWebhookSignatureFactory::class);
        ...
        static::assertSame(0 /*700 - 700*/ , $funding->getUser()->getCoins()); // no more coins
    ...
    public function test_bad_signature(): void
    {
        ...
        $mock = $this->getMockBuilder(VerifyWebhookSignature::class)
            ->disableOriginalConstructor()
            ->getMock();
        $mock->expects(static::once())->method('post')->willReturn(new VerifyWebhookSignatureResponse(['verification_status' => 'FAILURE']));
        $verifyWebhookSignature->setVerifyWebhookSignature($mock);
